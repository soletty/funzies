    ...
    async start(controller) {
      ...
      for (let attempts = 0; attempts < maxAttempts; attempts++) {
        const rows = await query<{
          ...
          "SELECT status, current_phase, raw_files, error_message FROM clo_screenings WHERE id = $1",
          [id]
        );

        if (rows.length === 0) {
          send({ status: "error", message: "Screening session not found" });
          controller.close();
          return;
        }
        ...
        if (status === "error") {
          send({ status: "error", message: error_message || "Screening generation failed" });
          controller.close();
          return;
        }
      ...
      send({ status: "error", message: "Timeout waiting for screening generation" });
      controller.close();
    },
  });
