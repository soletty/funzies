...
const ASSUMPTIONS_REGISTER: { domain: string; items: Assumption[] }[] = [
  {
    ...
    items: [
      ...
      { label: "Recovery rates static through projection", detail: "Per-position agency recovery rates apply to forward defaults when extracted from the source data; reinvested loans inherit the global rate (no per-deal recovery curve). Recovery percentages do not vary with the time of default — real recovery rates fall in downturns when defaults cluster, which the model does not capture.", impact: "medium" },
