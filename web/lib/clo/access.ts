...
function rowToHolding(row: Record<string, unknown>): CloHolding {
  return {
    ...
    premiumDiscountAmount: num(row.premium_discount_amount),
    discountAmount: num(row.discount_amount),
    premiumAmount: num(row.premium_amount),
    grossPurchasePrice: num(row.gross_purchase_price),
    ...
    defaultReason: (row.default_reason as string) ?? null,
    ...
    putDate: (row.put_date as string) ?? null,
    moodysIssuerRating: (row.moodys_issuer_rating as string) ?? null,
    moodysIssuerSrUnsecRating: (row.moodys_issuer_sr_unsec_rating as string) ?? null,
    spIssuerRating: (row.sp_issuer_rating as string) ?? null,
    fitchIssuerRating: (row.fitch_issuer_rating as string) ?? null,
    moodysSecurityRating: (row.moodys_security_rating as string) ?? null,
    spSecurityRating: (row.sp_security_rating as string) ?? null,
    fitchSecurityRating: (row.fitch_security_rating as string) ?? null,
    moodysRatingFinal: (row.moodys_rating_final as string) ?? null,
    spRatingFinal: (row.sp_rating_final as string) ?? null,
    fitchRatingFinal: (row.fitch_rating_final as string) ?? null,
    moodysDpRating: (row.moodys_dp_rating as string) ?? null,
    moodysRatingUnadjusted: (row.moodys_rating_unadjusted as string) ?? null,
    moodysIssuerWatch: (row.moodys_issuer_watch as string) ?? null,
    moodysSecurityWatch: (row.moodys_security_watch as string) ?? null,
    spIssuerWatch: (row.sp_issuer_watch as string) ?? null,
    spSecurityWatch: (row.sp_security_watch as string) ?? null,
    securityLevelMoodys: (row.security_level_moodys as string) ?? null,
    securityLevelSp: (row.security_level_sp as string) ?? null,
    securityLevel: (row.security_level as string) ?? null,
    lienType: (row.lien_type as string) ?? null,
    spPriorityCategory: (row.sp_priority_category as string) ?? null,
    spIndustryCode: (row.sp_industry_code as string) ?? null,
    moodysIndustryCode: (row.moodys_industry_code as string) ?? null,
    fitchIndustryCode: (row.fitch_industry_code as string) ?? null,
    kbraRating: (row.kbra_rating as string) ?? null,
    ...
    isSovereign: (row.is_sovereign as boolean) ?? null,
    isEnhancedBond: (row.is_enhanced_bond as boolean) ?? null,
    isCurrentPay: (row.is_current_pay as boolean) ?? null,
    isInterestOnly: (row.is_interest_only as boolean) ?? null,
    isPrincipalOnly: (row.is_principal_only as boolean) ?? null,
    accretionFactor: num(row.accretion_factor),
    capitalizationPct: num(row.capitalization_pct),
    aggregateAmortizedCost: num(row.aggregate_amortized_cost),
    averageLife: num(row.average_life),
    dayCountConvention: (row.day_count_convention as string) ?? null,
    paymentPeriod: (row.payment_period as string) ?? null,
    cusip: (row.cusip as string) ?? null,
    facilityCode: (row.facility_code as string) ?? null,
    facilityId: (row.facility_id as string) ?? null,
    figi: (row.figi as string) ?? null,
    countryCode: (row.country_code as string) ?? null,
    servicer: (row.servicer as string) ?? null,
    servicerMoodysRating: (row.servicer_moodys_rating as string) ?? null,
    servicerSpRating: (row.servicer_sp_rating as string) ?? null,
    dealDefaultedBegin: (row.deal_defaulted_begin as string) ?? null,
...
function rowToTranche(row: Record<string, unknown>): CloTranche {
  return {
    ...
    trancheType: (row.tranche_type as string) ?? null,
    liabPrin: (row.liab_prin as string) ?? null,
    legalMaturityDate: (row.legal_maturity_date as string) ?? null,
    amountNative: num(row.amount_native),
    vendorCustomFields: (row.vendor_custom_fields as Record<string, unknown>) ?? null,
  };
}
...
function rowToTrancheSnapshot(row: Record<string, unknown>): CloTrancheSnapshot {
  return {
    ...
    ratingMoodysIssuance: (row.rating_moodys_issuance as string) ?? null,
    ratingSpIssuance: (row.rating_sp_issuance as string) ?? null,
    ratingFitchIssuance: (row.rating_fitch_issuance as string) ?? null,
    icInterest: num(row.ic_interest),
    accrualStartDate: (row.accrual_start_date as string) ?? null,
    accrualEndDate: (row.accrual_end_date as string) ?? null,
    baseRate: num(row.base_rate),
    unscheduledPrincipalPaydown: num(row.unscheduled_principal_paydown),
    dataSource: (row.data_source as string) ?? null,
  };
}
...
function rowToAccountBalance(row: Record<string, unknown>): CloAccountBalance {
  return {
    ...
    accountInterest: num(row.account_interest),
    dataSource: (row.data_source as string) ?? null,
  };
}
...
function rowToTrade(row: Record<string, unknown>): CloTrade {
  return {
    ...
    cashFlowType: (row.cash_flow_type as string) ?? null,
    bookDate: (row.book_date as string) ?? null,
    transactionCode: (row.transaction_code as string) ?? null,
    description: (row.description as string) ?? null,
    saleReason: (row.sale_reason as string) ?? null,
    trustAccount: (row.trust_account as string) ?? null,
    figi: (row.figi as string) ?? null,
