...
describe("A2 — computeReinvOcDiversion (lesser-of)", () => {
  ...
  it("deep breach → diverts the 50% cap, not the (larger) cure amount", () => {
    ...
    expect(diversion).toBeCloseTo(expectedCap, 2);
  ...
  it("zero available interest → zero diversion (no infinite loop)", () => {
    expect(computeReinvOcDiversion(0, 350_000_000, 350_000_000, 103.74, 50)).toBe(0);
    expect(computeReinvOcDiversion(-1, 350_000_000, 350_000_000, 103.74, 50)).toBe(0);
  });

  it("zero rated debt → zero diversion (no divide-by-zero)", () => {
    expect(computeReinvOcDiversion(10_000_000, 0, 0, 103.74, 50)).toBe(0);
  });
});
