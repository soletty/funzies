...
const DEDUP_KEYS: Record<string, (item: Record<string, unknown>) => string> = {
  ...
  trancheSnapshots: (t) => normClassLetter(t.className),
  accounts: (a) => norm(a.accountName),
  assetRateDetails: (a) => `${norm(a.obligorName)}|${norm(a.facilityName)}`,
  interestAmountsPerTranche: (i) => normClassLetter(i.className),
  ...
  ratingActions: (r) => `${norm(r.agency)}|${normClassLetter(r.tranche)}|${norm(r.actionType)}`,
  events: (e) => `${norm(e.eventType)}|${norm(e.eventDate)}`,
  spCdoMonitor: (s) => normClassLetter(s.tranche),
