import type { CommitteeMember } from "../types.js";

const MEMBER_HEADING = /^##\s+Member\s+(\d+):\s*(.+?)\s*\|\s*(.+)$/i;
const SECTION_HEADING = /^###\s+(.+)$/;

export function parseCommitteeMembers(markdown: string): CommitteeMember[] {
  const members: CommitteeMember[] = [];
  const lines = markdown.split("\n");

  let current: Partial<CommitteeMember> | null = null;
  let currentSection = "";
...
function applySection(current: Partial<CommitteeMember>, sectionName: string, text: string) {
  const key = sectionName.toLowerCase();

  if (key.includes("background")) {
    current.background = text;
  } else if (key.includes("investment philosophy") || key.includes("philosophy")) {
