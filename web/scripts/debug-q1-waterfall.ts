 ...
 *
 * Note: this script reads a pre-canned `ResolvedDealData` snapshot from
 * disk and never invokes `resolveWaterfallInputs`, so there are no
 ...
 * the resolver's warnings array MUST be passed as the third argument
 * to every `buildFromResolved` call below — otherwise blocking
 * warnings would be silently bypassed and the script would run on
 * sentinel data. The cross-file scan in
 * `lib/clo/__tests__/incomplete-data-banner-bijection.test.ts` covers
 * `web/scripts/`, but it only catches a divergent inline
