...
- [KI-62 — PIK loans modeled incompletely — forward rate not extracted; bond T=0 par excludes historical PIK](#ki-62)
...
<a id="ki-62"></a>
### [KI-62] PIK loans modeled incompletely — forward PIK rate not extracted into engine; bond T=0 par excludes historical PIK

**PPM reference:** PIK loans (Tele Columbus, Financiere Labeyrie, etc.) carry a coupon structure with two legs: a cash-pay leg (paid each period in cash) and a PIK leg (capitalized to par each period). The SDF reports the PIK leg's annual rate per facility via the `Current_Facility_Spread_PIK` column (Asset_Level CSV column 88) as a decimal (e.g. `0.01` = 1.0% per annum). Cumulative historical PIK already absorbed into par is reported via `PIK_Amount` (denormalized facility-level total).

**Background — what was attempted and reverted:** A previous attempt (commit fc07cbf, reverted in 213f71e) added a binary `isPik: boolean` engine dispatch that routed every PIK loan's full computed accrual to `survivingPar` when `isPik=true`. The binary boolean is structurally insufficient: real PIK loans have *separate* cash and PIK rates. The dispatch erroneously used `all_in_rate` (the cash leg) to compute par accretion (over-routing) and zeroed cash interest on split-coupon loans (under-routing). The revert preserves the original bug shape (no PIK accretion, full coupon to cash) but is at least consistent and audit-able. KI-62 captures the actual gap with the right data shape.

**Two sub-fixes, same KI:** Both attack different ends of the timeline of how PIK loans are represented in the engine. Sub-fix A handles forward accretion (engine misses ongoing PIK); Sub-fix B handles historical accretion at T=0 (bond par_balance excludes already-accreted PIK). Same source-data family, same downstream invariants (par balance, OC numerator, MV calc). Shipping in stages: A first (no verification gap), then B once Commitment semantics are confirmed.

---

#### Sub-fix A — Forward PIK accretion

**Current engine behavior:** `web/lib/clo/projection.ts:2227-2275` per-loan accrual loop computes only the cash-leg interest: `par × all_in_rate × dayFrac` for floaters or `par × fixedCouponPct × dayFrac` for fixed. All flows to `interestCollected`. No PIK accretion. `Current_Facility_Spread_PIK` is not extracted — the column is unread anywhere under `web/lib/clo/`.

**Source data verified by trustee transaction cross-check (2026-04-01 cycle):**
- Financiere Labeyrie Fine Foods (Facility B, 2 slices, par=5,028,101.56, all_in_rate=7.127%, **Current_Facility_Spread_PIK=0.01 = 1.0%/yr**, pikAmount=28,101.56). Trustee paid €88,010.59 cash interest on 2026-03-31 (SDF Transactions ECB.csv) on €5M native principal. Math: `5,000,000 × 0.07127 × 89/360 = 88,011.94` — matches paid amount to the cent. **Confirms `all_in_rate` represents the cash leg only**; `Current_Facility_Spread_PIK` is the additive PIK leg on top. The PPM-correct annual economic rate on this loan is therefore `7.127% cash + 1.0% PIK = 8.127%` total.
- Tele Columbus AG ("TCGR 10.0 PIK 1st lien Nts due 2029 REGS", 4 slices, par_balance sum=2,500,000, all_in_rate=0, **Current_Facility_Spread_PIK=0**, pikAmount=581,032.34). PIK toggle currently OFF — no forward accretion. The 581,032.34 is *historical* cumulative PIK already accreted into the per-row par (handled by Sub-fix B below). No transaction row in the period — consistent with all_in_rate=0.
- Across all ~200 Euro XV positions: **Financiere is the ONLY position with non-zero `Current_Facility_Spread_PIK`.** The forward-accretion gap is scoped to one facility on this deal.

**PPM-correct behavior — additive PIK accretion:** Each period, in addition to cash interest:
```
pikAccretion = loanBegPar × (pikSpreadBps / 10000) × dayFrac
loan.survivingPar += pikAccretion              (post-default surviving par)
```
The PIK is purely additive — never subtracts from cash leg, never re-routes the existing accrual. `loanBegPar` is the same pre-default-pre-maturity capture the cash interest path uses (line 2082); convention is "PIK accrues on pre-default pre-maturity par; the accreted PIK survives onto the post-default surviving par share, or is paid out at maturity."

**Quantitative magnitude (post-revert, Euro XV today):**
- Financiere only: forward under-count of par accretion = `5,028,101 × 0.01 × 91/360 ≈ €12,710/quarter`. Over 13 quarters to maturity (Jul 2029) ≈ €165K cumulative shortfall in Financiere par. Cascades into pool par (small Δ), mgmt-fee base, equity residual, n1 sub-distribution drift.
- Everyone else: zero forward error.

**Path to close (sub-fix A):**
1. **DB migration** — add `clo_holdings.pik_spread_bps numeric` column.
2. **Parser** (`sdf/parse-asset-level.ts`): extract `Current_Facility_Spread_PIK` (decimal e.g. 0.01) → multiply by 10,000 → store as `pik_spread_bps`. Magnitude validate (`> 1500` → reject as locale mis-parse; `< 0` → reject as sign violation). Anti-pattern #5 boundary discipline.
3. **Type plumbing** — add `pikSpreadBps?: number | null` to `CloHolding` (`types/entities.ts`); add `pikSpreadBps?: number` to `ResolvedLoan` (`resolver-types.ts`); add `pikSpreadBps?: number` to `LoanInput` and `LoanState` (`projection.ts`).
4. **Resolver** (`resolver.ts`): propagate `pikSpreadBps` from holding to ResolvedLoan. Three-tier blocking ladder (anti-pattern #3):
   - `pikSpreadBps < 0` → block (sign invariant).
   - `pikSpreadBps > 1500` → block (implausibility — distressed Euro CLO PIK margins top ~10-12%; 15% is the hard ceiling).
   - `pikAmount > 0 AND pikSpreadBps == null` → block (extraction gap on a position whose source data demonstrates PIK structure).
   - `pikSpreadBps == 0 AND pikAmount > 0` → propagate as 0 (Tele Columbus shape; PIK toggle off, historical accretion only).
5. **Engine** (`projection.ts` per-loan accrual loop, after the existing cash interest accumulation): if `loan.pikSpreadBps != null && loan.pikSpreadBps > 0`:
   ```
   const pikAccretion = loanBegPar × (loan.pikSpreadBps / 10000) × dayFrac;
   if (q === loan.maturityQuarter) {
     // Maturity convention: skip accretion on the maturing period —
     // simpler than threading PIK into the maturity payout, and the
     // economic delta is one period's PIK once at end-of-life.
   } else {
     loan.survivingPar += pikAccretion;
   }
   ```
6. **Switch-simulator** (`switch-simulator.ts`): change pctPik recompute driver from `is_pik` (= `pikAmount > 0`) to `pikSpreadBps > 0`. Semantic: "actively accreting PIK," not "structurally PIK eligible." Document and pin via existing d4 test.
7. **Synthetic tests** (bijection rule — ship in same PR):
   - Split-margin baseline: `par=100M, all_in_rate=7%, pikSpreadBps=100` → cash collected = `par × 7% × dayFrac`, par_growth = `par × 1% × dayFrac`.
   - Toggle-off: `pikSpreadBps=0, pikAmount>0` (Tele Columbus shape) → zero accretion, no block, propagation through resolver succeeds.
   - Maturity invariant: PIK loan maturing at Q5 → final-period accretion is *not* applied (maturity-skip convention). Synthetic loan with maturity at q=2; assert `survivingPar` is fully zeroed at q=2 with no zombie PIK.
   - Default invariant: PIK loan with 50% partial default at period start → PIK accretes on full pre-default par, additive, ends up on surviving 50%. Pins the convention "cash and PIK both use loanBegPar pre-default."
   - Resolver three-tier gate: `pikAmount>0, pikSpreadBps=null` → blocking warning fires; `pikAmount=0, pikSpreadBps=null` → no block.
   - n1 cascade re-baseline: Financiere alone shifts pool par by +€12,710/quarter. Pin the n1 sub-distribution drift's new value (currently -€57,239.62; expected to shift slightly more positive as PIK accretion adds to pool par → marginally more interest collected → smaller residual to subs).

---

#### Sub-fix B — Bond T=0 par_balance excludes historical PIK

**Current parser behavior:** `web/lib/clo/sdf/parse-collateral.ts:131-136`:
```
const parBalance = (fundedBalance > 0) ? fundedBalance : principalBalance;
```
For Loans (`Principal_Funded_Balance > 0` per SDF convention): `parBalance = Principal_Funded_Balance`, which already includes accreted PIK (Financiere row 1: `Principal_Funded_Balance = 3,016,860.94 = 3,000,000 + 16,860.94 slice's PIK share`).

For Bonds (`Principal_Funded_Balance = 0` per SDF convention; bonds carry no funded/unfunded distinction): `parBalance = Principal_Balance`, which is the raw principal **without** historical PIK. Tele Columbus 4 slices: per-row `par_balance = Principal_Balance ∈ {1M, 500K, 643K, 357K}` summing to 2.5M; the per-row `Commitment ∈ {1.232M, 616K, 792K, 440K}` sums to 3.081M, with the diff per slice equaling that slice's proportional share of facility-level `pikAmount = 581,032.34`.

**Magnitude:** Tele Columbus pool par contribution under-counts by €581,032 (~23% under-count on this position's true par). Cascades:
- Pool total par: under-count by €581K (~0.12% of €493M Euro XV pool).
- WAS denominator: same shape; small Δ.
- OC numerator: under-count by €581K (real numerator is `Principal_Balance + accreted_PIK`).
- MV calc: `marketValue × 58.5% × par` where par is under-counted; per-row MV is under-stated.
- T=0 IRR / equity book value: cascades.

**Verification gap blocking sub-fix B:** Need to confirm `ECB.Commitment` semantics for bonds. Two possible interpretations:
- **(i) Live**: `Commitment` grows as PIK accretes. Each period's report shows updated Commitment = original_face + cumulative_PIK_to_date. Fix: `parBalance = Commitment` for bonds.
- **(ii) Static**: `Commitment` is a fixed face value at issue that *coincidentally* equals `Principal_Balance + pikAmount` on this snapshot. Fix: `parBalance = Principal_Balance + slice_share_of_pikAmount`.

The DB has only one period per deal (per CLAUDE.md); cross-period verification requires either a prior-period BNY trustee monthly report PDF for Tele Columbus or the SDF column-dictionary spec. Until verified, sub-fix B is **deferred**; sub-fix A is independently shippable.

**Path to close (sub-fix B, post-verification):**
1. Verify Commitment semantics via prior-period trustee PDF or SDF spec.
2. Update `parse-collateral.ts:131-136` per the verified interpretation.
3. Re-baseline Tele Columbus pool-par contribution; cascade-validate against Euro XV trustee MV / OC numerator at T=0.
4. Add a parser unit test pinning the bond branch.

---

**Deferral rationale:** Open. Sub-fix A is live wrong-number on Euro XV (Financiere). Sub-fix B has live wrong-number magnitude on Euro XV (Tele Columbus 581K) but is blocked on data-semantics verification. Shipping order: A first, then B once verified.

**Test:** Sub-fix A markers (synthetic split-margin baseline, toggle-off, maturity invariant, default invariant, three-tier gate, n1 cascade) ship with the sub-fix A PR. Sub-fix B parser test ships with the sub-fix B PR. Both removed at full closure of the entry per closed=deleted doctrine.
