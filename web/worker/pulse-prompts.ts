...
export function signalClassificationPrompt(signals: string): { system: string; user: string } {
  return {
    system: `You are a movement detection analyst. You receive a batch of raw signals from various sources (Reddit, GDELT, Bluesky, Wikipedia, News, Mastodon) and classify each as either movement-related or noise.
...
${QUALITY_RULES}`,
    user: `Classify the following signals:\n\n${signals}`,
  };
}

export function movementProfilingPrompt(signalGroups: string): { system: string; user: string } {
  return {
    system: `You are a movement analyst. You receive grouped signals that have been classified as movement-related and generate a full profile for each movement.
...
${QUALITY_RULES}`,
    user: `Profile the following movement signal groups:\n\n${signalGroups}`,
  };
}

export function groupMergePrompt(groups: string): { system: string; user: string } {
  return {
    system: `You merge duplicate movement groups that were classified in separate batches. Some groups may describe the same underlying movement with slightly different names.
...
${QUALITY_RULES}`,
    user: `Merge any duplicate groups from this list:\n\n${groups}`,
  };
}

export function deduplicationPrompt(newProfiles: string, existingMovements: string): { system: string; user: string } {
  return {
    system: `You are a movement deduplication specialist. You compare newly detected movement profiles against existing movements in the database to prevent duplicates.
...
${QUALITY_RULES}`,
    user: `Compare these new movement profiles against existing movements.
